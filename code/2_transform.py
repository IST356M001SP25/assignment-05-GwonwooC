...
col_data_list = [pd.read_csv(f'cache/col_{year}.csv') for year in survey_data['year'].unique()]
col_data = pd.concat(col_data_list, ignore_index=True)
...
salary_by_location_age = combined.pivot_table(index='_full_city', columns='How old are you?', values='_annual_salary_adjusted', aggfunc='mean')
salary_by_location_age.to_csv('cache/annual_salary_adjusted_by_location_and_age.csv')

salary_by_location_education = combined.pivot_table(index='_full_city', columns='What is your highest level of education completed?', values='_annual_salary_adjusted', aggfunc='mean')
salary_by_location_education.to_csv('cache/annual_salary_adjusted_by_location_and_education.csv')
